...
export class Sitting extends State {
    ...
    enter() {
        this.game.player.frameX = 0;      // fix a blank bug
...
export class Diving extends State {
    ...
    enter() {
        ...
        this.game.player.maxFrame = 6;
        this.game.player.frameY = 6;
        this.game.player.vy = 15;     // massive push down attack
    }
    handleInput(input) {
        this.game.particles.unshift(new Fire(this.game, this.game.player.x + this.game.player.width * 0.5, this.game.player.y + this.game.player.height * 0.5)); // ajout des particule fire
        if (this.game.player.onGround()) {
            this.game.player.setState(states.RUNNING, 1);
            for (let i = 0; i < 30; i++) {
